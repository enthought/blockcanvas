...
from enthought.util.networkx_ import CyclicGraph, graph_map
...
class BlockTestCase(unittest.TestCase):
    ...
    def _base(self, code, inputs, outputs, conditional_outputs,
        ...
        def num_to_block(x):
            ...
            try:
                ...
                return b
        dep_graph = graph_map(num_to_block, dep_graph)
        ...
        if b.sub_blocks is not None:
            ...
            b_dep_graph = DiGraph()
            for i in b.inputs:
                b_dep_graph.add_edge(b, i)
            for o in b.all_outputs:
                b_dep_graph.add_edge(o, b)

        # Compare dep_graphs
        self.assertEqual(b_dep_graph.adj, dep_graph.adj)
    ...
    def _block_structure(cls, b):
        ...
        if isinstance(b, Block):
            ...
            g = b._dep_graph is not None and b._dep_graph or DiGraph()
            return frozenset((
                repr(b.ast),
                tuple(map(frozenset, (
                    b.inputs,
                    ...
                    map_values(
                        frozenset, graph_map(node_contents, g).adj).items(),
                ))),
            ))
        else:
            return b
