...
# Geo library imports
from geo.testing.api import BasicTestMappingProtocol

def adapt_keys(context):
    ''' Wrap a context so that it accepts all key types used by
        BasicTestMappingProtocol.

        IDataContext is almost a mapping object, except it rejects keys that
        aren't strings. We want to reuse python's dict test cases, so we wrap
