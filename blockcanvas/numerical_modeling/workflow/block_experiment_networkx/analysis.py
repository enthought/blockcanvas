...
def dependency_graph(asts, to_ast=lambda x: x):
    ''' Compute the dependency graph for a set of ASTs.
        ...
        dependency graph won't determine a well-defined program. e.g.
            ...
            >>> g = dependency_graph(to_ast=parse, asts=[
            ...     'a = 1', 'a = 2'
            ... ])
            >>> sorted( g.nodes() )
            ['a = 1', 'a = 2']
        ...
        Some examples:
            ...
            >>> assert sorted( dependency_graph(to_ast=parse, asts=[
            ...     'a = b+c', 'c = 3', 'b = f(c)'
            ... ]).edges() ) == [
            ...     ('a = b+c', 'b = f(c)'),
            ...     ('a = b+c', 'c = 3'),
    ...
    g = graph_map(lambda ast: d[ast], _dependency_graph(list(d)))

    if is_directed_acyclic_graph(g):
        return g
    else:
        raise CyclicGraph(g.adj)
...
class NameFinder:
    ...
    def __init__(self, free=(), locals=(), conditional_locals=(), globals=()):
        ...
        # Consider magically bound names as global to anything # XXX Hack
