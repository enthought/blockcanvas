...
class BlockCanvasButton(Button):

    canvas = Instance(BlockCanvas)

    graph_controller = Property

    def _get_graph_controller(self):
        return self.canvas.graph_controller


class CollapseAllButton(BlockCanvasButton):
...
class ExpandAllButton(BlockCanvasButton):
...
class RelayoutButton(BlockCanvasButton):
